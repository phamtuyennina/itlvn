<?php  if(!defined('_source')) die("Error");	
	...
	if(!empty($_POST)){	
		...
		$d->reset();
		$sql = "select phivanchuyen from #_place_city where id=".$_POST['thanhpho']."";
		$d->query($sql);
		$phivanchuyen12 = $d->fetch_array();
		...
		$phivanchuyen=$phivanchuyen12['phivanchuyen'];
		...
		if ($coloi==FALSE) 
		{
			...
			$mahoadon = date('dmY').'CD'.$songaunhien;
			$sql = "INSERT INTO  table_donhang (madonhang,hoten,dienthoai,diachi,email,httt,tonggia,thanhpho,quan,phuong,noidung,ngaytao,tinhtrang,nguongoc,ngaycapnhat,htgh,ip,export,id_user,phivanchuyen) 
			  VALUES ('$mahoadon','$hoten','$dienthoai','$diachi','$email','$httt','$tonggia','$thanhpho','$quan','$phuong','$noidung','$ngaydangky','1','$nguongoc','$ngaycapnhat','$htgh','$ip','1','$id_user','$phivanchuyen')";	
		...
		if(mysql_query($sql))#Nếu insert bảng đơn hàng thành công
		{
			...
			if(is_array($_SESSION['cart']))
			{
				...
				foreach($_SESSION['cart'] as $k=>$v){
					...
					if($q==0) continue;
					$sql1="UPDATE table_product set luotmua=luotmua+".$q." where id=".$pid."";
					mysql_query($sql1);
				...
				if($co == true)
				{
					...
					*=====================================*/
					...
					$mail->CharSet = "utf-8";
					$header.='<table width="100%" border="0" style="border-collapse: collapse;"><tr><td colspan="6"><table style="width:100%;background: #333; color: #FFF"><tr>';
			$header.='<td style="padding:7px; text-align:left;width:75%;">';
			$header.='<img src="http://'.$config_url.'/'._upload_hinhanh_l.$row_banner['photo'].'" alt="'.$company['ten'].'" style="width:200px;margin-left:10px;" />';
			$header.='</td>';
			$header.='<td style="padding:7px; text-align:left;width:25%">';
			$header.='<p><img src="http://nina.net.vn/phone_2_icon.png" style="float:left;margin-right:10px;" />';
			$header.='<strong style="display:block;">Hotline:</strong>';
            ...
            $footer = '';
            ///Footer mail
            $footer.='<tr><td colspan="6"><table style="width:100%;border-top:1px solid #CCC; background: #333; color: #FFF"><tr>';
			...
			$body = '<tr><td colspan="6">
				<p>Cảm ơn quý khách đã đặt hàng dưới đây là thông tin đơn hàng của quý khách:</p></td></tr>
				  ...
				  <tr style="height:40px; padding:10px;">
					<td colspan="3" style="line-height:40px;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;">Mã đơn hàng: <b>' .$mahoadon. '</b></td>
					<td colspan="3" style="border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;line-height:25px;">Ngày đặt hàng: <b>' . date('d/m/Y', time()) . '</b><br />
						Trạng thái đơn hàng: Mới đặt<br/>
					</td>
				  </tr>
				  <tr>
					<td colspan="6" style="line-height:25px;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;">
						...
						<tr>
							<td style="text-align:center;width:10%;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;"><b>Mã sản phẩm</b></td>
			...
			## Tao du lieu don hang				
				$max=count($_SESSION['cart']);
					...
					if($q==0) continue;
					 ...
					 $body.='<tr>
							<td style="text-align:center;width:10%;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;">' . $info['masp'] . '</td>
							...
							<td style="width:30%;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;">
								<table>
									...
									if($mausac!=''){
										$body.='<tr><td> Size :' . getSizeById($size). '</td></tr>';}
								$body.='</table>
							</td>							
							<td style="text-align:center;width:15%;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;">' . number_format(get_price($info['id']), 0, ',', '.') . '&nbsp;VNĐ</td>
							<td style="text-align:center;width:15%;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;">' . $q . '</td>
							<td style="text-align:center; width:20%;border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;">' . number_format(get_price($info['id']) * $q, 0, ',', '.') . '&nbsp;VNĐ</td>
						</tr>';

		}


		 $body.='<tr><td colspan="6" style="text-align:right; border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;" >Tổng cộng: <b>' . number_format(get_order_total(), 0, ',', '.') . '&nbsp;VNĐ</b></td></tr>';                

        $body.='<tr><td colspan="6" style="text-align:right; border: 1px solid #E9E9E9;padding: 5px;background-color: #FFF;" >Tổng thanh toán: <b>' . number_format(get_order_total(),0, ',', '.') . '&nbsp;VNĐ</b></td></tr>';
